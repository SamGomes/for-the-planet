...
analyse <- function(gameresultslog, subfolder, adversary){
	...
	plot <- ggplot(agg, aes(x = agg$roundId, y=agg$playerInvestEnv, group=agg$playerName, color=agg$playerName, alpha=aggJ$survivalRate)) 
	plot <- plot + geom_line(stat = "identity", size=1.2)
	plot <- plot + geom_point(stat = "identity", size=2.0 ) + scale_alpha(range = c(0.3, 1))
	# plot <- plot + geom_errorbar(width=.1, aes(ymin=agg$playerInvestEnv - aggSD$playerInvestEnv, ymax=agg$playerInvestEnv + aggSD$playerInvestEnv, fill=agg$playerName, alpha=0.8))
	plot <- plot + labs(x = "Round Id", y = "Cooperation Investment", color="Player Type") + theme(axis.text = element_text(size = 15), axis.title = element_text(size = 15, face = "bold")) #+ scale_group_discrete(labels = as.character(c("Constructive\nCollectivist","Constructive\nIndividualist","Disruptive\nCollectivist","Disruptive\nIndividualistic","Random")))  
	...
	# agg <- melt(feltEmotionsLog2, id.vars = c("roundId","playerName"), measure.vars = varsToConsider)
	# agg <- agg[agg$value != 0,]
